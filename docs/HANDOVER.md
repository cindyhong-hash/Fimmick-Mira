...
**基準：360 tests / 359 pass。唯一允許的失敗是 `planner/content-brief` 的 `subtitleText: null`（改動前就存在）。出現第二個失敗就是改壞了。**
...
- **icon 不含文字**：中文交給排版階段用字型渲染。理由是圖像模型畫中文會缺筆畫
  （本專案已踩過），這是與使用者確認後的決定
- 一個賣點一張（3–5 張），預設不勾選（一次會是 3–5 張付費）
- `deriveBenefitPoints()` 從 `Product.description` 規則拆解，不另打 LLM
