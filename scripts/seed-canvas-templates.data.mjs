...
/**
 * 示意商品有好幾支，每個版挑色系合的那一支。
 *
 * 一開始十七個版共用同一支藍瓶，結果墨綠底配藍瓶很突兀——示意圖的用途是
 * 「讓人看出這個版長怎樣」，配色不合就失去意義。這幾支是去背 PNG。
 */
const PRODUCTS = {
  blue: `${BLOB}product-set-hero-1789962056557-1ocqi87cfc1h.png`,   // 淺藍壓瓶
  // 這四支都已裁掉透明邊。去背出來的 PNG 四周留了大片空白（商品只佔畫布 10–14%），
  // 圖層是依原比例 contain 進框的，不裁的話「巨型商品」那種版會縮成一小支。
  green: `${BLOB}1790063738911-aqpn0pb29lt.png`,                    // 墨綠玻璃滴管瓶
  white: `${BLOB}1790063739549-gn2sp249th.png`,                     // 白色霧面軟管
  amber: `${BLOB}1790063740119-w6y353zypt.png`,                     // 琥珀玻璃瓶
  pink: `${BLOB}1790063740715-y2z16zkdeci.png`,                     // 粉色霜罐
};
