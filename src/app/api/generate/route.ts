...
import { generateImageFal, generateImageFluxSchnell, describeStyle, describeProduct, editImageFal, removeBackground } from "@/lib/fal";
...
import { compositeImage, overlayLogo, compositeCollage, overlaySubImageCard, overlayProduct, extractDominantColor, sampleCornerBusyness, sampleRegionBusynessByZone, type SubCardVariant } from "@/lib/composite";
...
// 副圖統一卡片元件：左上彩色膠囊標籤＋粗體標題＋細字副標，所有副圖共用同一組件
const SUB_IMAGE_CARD_TEMPLATE = `SUB-IMAGE CARD COMPONENT (apply identically to every sub-image, only text content changes):
- Top-left: a solid-color pill/badge (rounded rectangle, ~90% opacity, brand accent color), containing a short label in FONT A, all-caps or bold, white text.
- Below the badge: headline in FONT A, bold, dark color, max 2 lines.
- Optional: one accent-color highlight word/price in the headline line (e.g. brand red/orange), same FONT A weight, just different color.
- Subtitle line (if present) in FONT B, regular weight, smaller, muted gray.
- Same badge shape, same corner radius, same padding, same text block position (top-left anchored) across ALL sub-images — this is one reusable UI component, not redesigned per cell.`;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ══ 多圖分支：layoutId !== "single" → 逐格生成 + 拼版（與單圖完全隔離）══
    const multiLayoutId = activity.layoutId || "single";
    if (multiLayoutId !== "single") {
      ...
      const size = SIZE[ratio] ?? { w: 1024, h: 1024 };
...
${cellNoProduct}${cellProductFreeNote}${razorExclusionNote}${subImageNoText}`;
    ...
    await db.activity.update({ where: { id: activityId }, data: { status: "FAILED", errorMessage } });
    return NextResponse.json({ error: String(err), errorMessage }, { status: 500 });
  }
}

/** 從各種錯誤（含 OpenRouter/供應商回傳的 JSON 字串）萃取「人看得懂」的原因，存進 DB 顯示。 */
function extractErrorMessage(err: unknown): string {
  let raw = err instanceof Error ? err.message : String(err);
  // 供應商錯誤常是 `400 {"error":{"message":"..."}}` → 抓內層 message
  ...
  // 常見額度問題 → 給更明確的中文提示
  if (/credit balance|insufficient_quota|too low|quota/i.test(raw)) {
    return `API 額度不足：${raw}`.slice(0, 500);
  }
