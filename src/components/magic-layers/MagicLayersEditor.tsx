...
type EL = {
  ...
  isArt?: boolean;           // 由 AI 特效字生成的圖片圖層（可再用 AI 微調）
...
export type SavedLayer = {
  ...
  isArt?: boolean;                    // image layer generated by AI 特效字（可再 AI 微調）
  isText?: boolean; text?: string; color?: string; fontSize?: number; fontFamily?: string; fontWeight?: number; align?: "left" | "center" | "right";
...
export function MagicLayersEditor({ image, layers, fragmentation, backgrounds, logos, name, onRename, onBack, onSave }: { image: HTMLImageElement; layers: LayerData[]; fragmentation?: FragmentationReport; backgrounds?: { url: string; label?: string }[]; logos?: string[]; name?: string; onRename?: (name: string) => void; onBack?: () => void; onSave?: (payload: { docW: number; docH: number; layers: SavedLayer[]; imageDataUrl: string; finalize: boolean }) => Promise<void> }) {
  ...
  const serializeLayers = useCallback((): SavedLayer[] => layersRef.current.map((l, i) => ({
    ...
    ...(l.isText
      ...
      : l.shape
        ? { shape: { ...l.shape } }
        : { image: l.src ?? (l.canvas ? (safeDataUrl(l.canvas) ?? undefined) : undefined), ...(l.isArt ? { isArt: true, text: l.text } : {}) }),
  })), []);
  ...
  // AI 微調：拿現有藝術字圖層 + 一句指令，叫 AI 在原圖基礎上改（保留同樣的字），換成新版本
  const editArtText = async () => {
    ...
    setArtBusy(true);
    try {
      const r = await fetch("/api/magic-layers/arttext", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: target.text, width: Math.round(target.w), height: Math.round(target.h), editImageUrl: target.src, instruction }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error ?? r.statusText);
      const canvas = await loadToCanvas(d.url);
      if (!canvas) throw new Error("讀取特效字失敗");
                ...
                {selEl.isArt && (
                  <>
                    ...
                    <button onClick={editArtText} disabled={artBusy || !artEdit.trim()}
                      style={{ width: "100%", marginTop: 8, height: 36, borderRadius: 10, border: "none", color: "#fff", fontSize: 13, fontWeight: 700, cursor: artBusy || !artEdit.trim() ? "default" : "pointer", background: artBusy ? "#a78bfa" : !artEdit.trim() ? "#c4b5fd" : "linear-gradient(135deg,#8b5cf6,#7c3aed)" }}>
                      {artBusy ? "AI 修改中…（約 15–30 秒）" : "✨ 依指令修改"}
                    </button>
                    <p style={{ margin: "6px 0 0", fontSize: 11, color: "#9ca3af", lineHeight: 1.5 }}>在目前這張藝術字上照你的指令修改，文字內容不變（可用復原還原）。</p>
                    <div style={{ height: 1, background: "#e5e7eb", margin: "18px 0" }} />
                  </>
                )}
