...
// 文字樣式（含 AI Style Recipe 映射進來的欄位）。全部在 canvas 即時渲染 → 中文字永遠由字體引擎畫，不會破字。
//  strokeW/underlay 位移/陰影位移皆為「佔字級 fs 的比例」，隨字放大縮小。
export type TextFx = {
  gradient?: [string, string] | null;   // 主填色漸層（上→下）
  strokeColor?: string; strokeW?: number;   // 外框
  shadow?: boolean;                      // 舊版簡易陰影（相容）
  shadowColor?: string; shadowDx?: number; shadowDy?: number; shadowBlur?: number;   // 進階陰影（比例）
  underlay?: { color: string; dx: number; dy: number } | null;   // 第二層位移副本（海報常見的立體/雙色）
  skew?: number;                         // 水平傾斜角度（度）
  italic?: boolean; letterSpacing?: number;
};
...
type EL = {
  ...
  isArt?: boolean;           // 舊版：由 AI 生成的藝術字「圖片」圖層（image-to-image）
  artStyled?: boolean;       // 新版：套了 AI Style Recipe 的「可編輯文字」圖層（字形永遠正確）
  artRefImage?: string | null;   // 生成時用的風格參考圖（data URL；供 reload 後續編/微調）
  artRecipe?: StyleRecipe | null;   // 目前套用的 Style Recipe（供 AI 微調在其上調整）
...
export type SavedLayer = {
  ...
  isArt?: boolean;                    // 舊版 AI 藝術字「圖片」圖層
  artStyled?: boolean;                // 新版：套了 AI Style Recipe 的可編輯文字
  artRefImage?: string;               // 生成時用的風格參考圖（data URL）
  artRecipe?: StyleRecipe;            // 目前套用的 Style Recipe
  isText?: boolean; text?: string; color?: string; fontSize?: number; fontFamily?: string; fontWeight?: number; align?: "left" | "center" | "right";
  fx?: TextFx | null;
  shape?: { kind: "rect" | "ellipse" | "line" | "icon"; fill: string; stroke: string; strokeWidth: number; radius?: number; icon?: string };
};

/** AI 從參考圖分析出的「Style Recipe」（純樣式，不含文字內容）。位移/寬度為佔字級比例。 */
export type StyleRecipe = {
  fontCategory?: string;                 // serif/ming | sans | rounded …（決定字體）
  color?: string;                        // 主填色（無漸層時）
  gradient?: string[] | null;            // 主填色漸層 [from,to]
  strokeColor?: string | null; strokeWidth?: number;    // 外框（0~0.14）
  shadowColor?: string | null; shadowDx?: number; shadowDy?: number; shadowBlur?: number;   // 陰影
  secondaryColor?: string | null; secondaryDx?: number; secondaryDy?: number;   // 第二層位移副本
  skew?: number;                         // 水平傾斜（度）
};
const FONT_CSS_MAP: Record<string, string> = {
  serif: "'Noto Serif TC',serif", ming: "'Noto Serif TC',serif", song: "'Noto Serif TC',serif",
  sans: "'Noto Sans TC',system-ui,sans-serif", gothic: "'Noto Sans TC',system-ui,sans-serif",
};
const clamp = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));
/** Style Recipe → 編輯器可套用的文字樣式（fx 全部即時 canvas 渲染，字形不受影響）。 */
function recipeToStyle(r: StyleRecipe): { color?: string; fontFamily?: string; fx: TextFx } {
  const fx: TextFx = {};
  if (Array.isArray(r.gradient) && r.gradient.length >= 2) fx.gradient = [r.gradient[0], r.gradient[r.gradient.length - 1]];
  if (r.strokeColor) { fx.strokeColor = r.strokeColor; fx.strokeW = clamp(Number(r.strokeWidth) || 0.06, 0.02, 0.16); }
  if (r.shadowColor) { fx.shadowColor = r.shadowColor; fx.shadowDx = clamp(Number(r.shadowDx ?? 0.04), -0.3, 0.3); fx.shadowDy = clamp(Number(r.shadowDy ?? 0.06), -0.3, 0.3); fx.shadowBlur = clamp(Number(r.shadowBlur ?? 0.04), 0, 0.4); }
  if (r.secondaryColor) fx.underlay = { color: r.secondaryColor, dx: clamp(Number(r.secondaryDx ?? 0.05), -0.4, 0.4), dy: clamp(Number(r.secondaryDy ?? 0.06), -0.4, 0.4) };
  if (r.skew) fx.skew = clamp(Number(r.skew), -20, 20);
  const cat = String(r.fontCategory ?? "").toLowerCase();
  const fontFamily = Object.keys(FONT_CSS_MAP).find((k) => cat.includes(k)) ? FONT_CSS_MAP[Object.keys(FONT_CSS_MAP).find((k) => cat.includes(k))!] : undefined;
  return { color: r.color && /^#|rgb/i.test(r.color) ? r.color : undefined, fontFamily, fx };
}
...
export function MagicLayersEditor({ image, layers, fragmentation, backgrounds, logos, name, onRename, onBack, onSave }: { image: HTMLImageElement; layers: LayerData[]; fragmentation?: FragmentationReport; backgrounds?: { url: string; label?: string }[]; logos?: string[]; name?: string; onRename?: (name: string) => void; onBack?: () => void; onSave?: (payload: { docW: number; docH: number; layers: SavedLayer[]; imageDataUrl: string; finalize: boolean }) => Promise<void> }) {
    ...
    (async () => {
      const els: EL[] = await Promise.all(layers.map(async (l) => {
        ...
        const el: EL = {
          ...
          artStyled: (l.meta?.artStyled as boolean | undefined) ?? false,
          artRefImage: (l.meta?.artRefImage as string | undefined) ?? null,
          artRecipe: (l.meta?.artRecipe as StyleRecipe | undefined) ?? null,
  ...
  const serializeLayers = useCallback((): SavedLayer[] => layersRef.current.map((l, i) => ({
    ...
    ...(l.isText
      ? { isText: true, text: l.text, color: l.color, fontSize: l.fontSize * (l.w / (l.naturalW || l.w)), fontFamily: l.fontFamily, fontWeight: l.fontWeight, align: l.align, ...(l.fx ? { fx: l.fx } : {}), ...(l.artStyled ? { artStyled: true, ...(l.artRefImage ? { artRefImage: l.artRefImage } : {}), ...(l.artRecipe ? { artRecipe: l.artRecipe } : {}) } : {}) }
      : l.shape
        ? { shape: { ...l.shape } }
  ...
  // 套用 Style Recipe 到「可編輯文字」圖層（維持 isText → 字形永遠由字體引擎渲染，不會破字）
  const applyRecipe = (target: EL, recipe: StyleRecipe, refImage: string | null) => {
    const { color, fontFamily, fx } = recipeToStyle(recipe);
    if (color) target.color = color;
    if (fontFamily) target.fontFamily = fontFamily;
    target.fx = fx;
    target.artStyled = true;
    target.artRecipe = recipe;
    target.artRefImage = refImage ?? target.artRefImage ?? null;
    target.thumb = makeThumb(target);
    markDirty(); render(); refresh();
  };
  // 生成藝術字：AI 分析參考圖 → Style Recipe → 套到目前文字（文字內容/字形完全不動）
  const generateRecipeInto = async (target: EL, refImage: string | null, instruction?: string) => {
    if (artBusy || !refImage) return;
    setArtBusy(true);
    try {
      const r = await fetch("/api/magic-layers/style-recipe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refImageUrl: refImage, instruction: instruction ?? undefined, currentRecipe: instruction ? target.artRecipe ?? undefined : undefined }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error ?? r.statusText);
      applyRecipe(target, d as StyleRecipe, refImage);
      setArtView("none"); setArtRef(null); setArtEdit("");
    } catch (err) { alert("AI 風格分析失敗：" + (err instanceof Error ? err.message : String(err))); }
    finally { setArtBusy(false); }
  };
  const applyArtText = () => { if (selEl && selEl.isText) generateRecipeInto(selEl, artRef); };        // State B「生成藝術字」
  const regenArt = () => { if (selEl && selEl.isText && selEl.artRefImage) generateRecipeInto(selEl, selEl.artRefImage); };   // 「重新生成」
  // AI 微調：在現有 recipe 上照指令調整（純參數，文字內容/字形不受影響）
  const editArtText = async () => {
    if (!selEl || !selEl.artStyled || artBusy) return;
    const instruction = artEdit.trim();
    if (!instruction) return;
    if (!selEl.artRefImage) { alert("找不到原始參考圖，請用「更換參考圖」重新指定。"); return; }
    setArtEdit("");
    await generateRecipeInto(selEl, selEl.artRefImage, instruction);
  };
              ...
              <div style={{ padding: 16, overflowY: "auto" }}>
                {selEl.isText && (artView === "setup" ? (
                  ...
                  <>
                    ...
                    <div style={{ height: 1, background: "#e5e7eb", margin: "18px 0" }} />
                    {selEl.artStyled ? (
                      /* 已套 AI 風格：可用 AI 微調 / 重新生成 / 更換參考圖（文字仍可直接編輯，字形永遠正確） */
                      <>
                        <div style={S.rhead}>AI 風格</div>
                        <textarea value={artEdit} disabled={artBusy} onChange={(e) => setArtEdit(e.target.value)} rows={2}
                          placeholder="描述想修改的地方…例：陰影再明顯一點、藍色再亮一些、更接近參考圖"
                        ...
                        <p style={{ margin: "6px 0 0", fontSize: 11, color: "#9ca3af", lineHeight: 1.5 }}>只調整風格參數；文字內容與字形不變（文字仍可直接編輯）。</p>
                        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                          <button disabled={artBusy || !selEl.artRefImage} onClick={regenArt} style={{ ...S.rbtn, flex: 1 }}>重新生成</button>
                          <label style={{ ...S.rbtn, flex: 1, textAlign: "center", cursor: artBusy ? "default" : "pointer" }}>
                            更換參考圖
                            <input type="file" accept="image/*" disabled={artBusy} style={{ display: "none" }}
                              onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; const fr = new FileReader(); fr.onload = () => { const el = selEl; if (el) generateRecipeInto(el, String(fr.result)); }; fr.readAsDataURL(f); e.target.value = ""; }} />
                          </label>
                        </div>
                      </>
                    ) : (
                ...
                {selEl.isArt && (
                  <>
                    {/* 舊版：AI 藝術字「圖片」圖層（image-to-image）。行為等同一般圖片圖層。 */}
                    <div style={S.rhead}>AI 藝術字（圖片）</div>
                    {selEl.thumb ? (
                    ...
                    <p style={{ margin: "8px 0 0", fontSize: 11, color: "#9ca3af", lineHeight: 1.5 }}>這是舊版圖片式藝術字。新版請用文字圖層的「AI 文字藝術字」（字形永遠正確、可再編輯）。</p>
                    <div style={{ height: 1, background: "#e5e7eb", margin: "18px 0" }} />
                  </>
                )}
...
function drawTextEl(ctx: CanvasRenderingContext2D, l: EL) {
  ...
  // 水平傾斜（海報斜體感）：以中心做 shear
  if (fx?.skew) { const k = Math.tan((fx.skew * Math.PI) / 180); ctx.transform(1, 0, -k, 1, 0, 0); }
  ...
  const outline = () => { if (fx?.strokeW && fx.strokeW > 0) { ctx.lineWidth = fs * fx.strokeW; ctx.strokeStyle = fx.strokeColor || "#ffffff"; ctx.lineJoin = "round"; ctx.miterLimit = 2; ctx.strokeText(l.text, tx, 0); } };
  const setShadow = (on: boolean) => {
    if (on && (fx?.shadowColor || fx?.shadow)) {
      ctx.shadowColor = fx?.shadowColor || "rgba(0,0,0,.4)";
      ctx.shadowBlur = fs * (fx?.shadowBlur ?? 0.1);
      ctx.shadowOffsetX = fs * (fx?.shadowDx ?? 0.03);
      ctx.shadowOffsetY = fs * (fx?.shadowDy ?? 0.06);
    } else { ctx.shadowColor = "transparent"; ctx.shadowBlur = 0; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0; }
  };
  // 第二層位移副本（雙色/立體）— 放最底、帶陰影
  if (fx?.underlay) {
    ctx.save();
    ctx.translate(fs * fx.underlay.dx, fs * fx.underlay.dy);
    setShadow(true); outline();
    ctx.fillStyle = fx.underlay.color; ctx.fillText(l.text, tx, 0);
    ctx.restore();
    setShadow(false);   // 主層不再重複陰影
  } else {
    setShadow(true);
  }
  // 主層：外框 + 填色
