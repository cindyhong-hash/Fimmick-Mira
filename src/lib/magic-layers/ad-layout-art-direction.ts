...
export function parseArtDirection(text:string):ArtDirectionDecision|null {
 ...
 try{
  ...
  if(!value||value.version!==1||!Array.isArray(value.directions)||value.directions.length!==3)return null;
  for(const d of value.directions){
   if(!d||typeof d!=="object"||typeof d.confidence!=="number"||!Number.isFinite(d.confidence)||d.confidence<0||d.confidence>1)return null;
